...
open SqlProvider.Transactions
open SqlProvider.Schema
open SqlProvider.Runtime
open SqlProvider.Common
open SqlProvider
...
type public SqlTypeProvider(config: TypeProviderConfig) as this =
    inherit TypeProviderForNamespaces(config, assemblyReplacementMap=["SqlProvider.DesignTime", "SqlProvider";
    ...
    let [<Literal>] SQL_PROVIDER = "Sqlprovider"
    ...
    let rec createTypes(connectionString, conStringName,dbVendor,resolutionPath,individualsAmount,useOptionTypes,owner,caseSensitivity, tableNames, contextSchemaPath, odbcquote, sqliteLibrary, rootTypeName) =
        ...
        let rootType, prov, con =
          lock lockObj3 (fun _ ->
            let rootType = ProvidedTypeDefinition(sqlRuntimeInfo.RuntimeAssembly,SQL_PROVIDER,rootTypeName,Some typeof<obj>, isErased=true)
    ...
    let paramSqlType = ProvidedTypeDefinition(sqlRuntimeInfo.RuntimeAssembly, SQL_PROVIDER, "SqlDataProvider", Some(typeof<obj>), isErased=true)
    ...
    do this.AddNamespace(SQL_PROVIDER, [paramSqlType])
