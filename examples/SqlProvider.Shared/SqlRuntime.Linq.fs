...
module internal QueryImplementation =
    ...
    let executeQuery (dc:ISqlDataContext) (provider:ISqlProvider) sqlExp ti =
        ...
        if (
            // provider.GetType() <> typeof<Providers.MSAccessProvider>
            provider.GetType() <> typeof<Providers.OracleProvider>
            ) then con.Close() //else get 'COM object that has been separated from its underlying RCW cannot be used.'
        results
